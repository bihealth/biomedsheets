...
def load_sheet():
    ...
    path = os.path.join(os.path.abspath(os.path.dirname(__file__)), "example_cancer_matched.json")
    sheet_json = io.json_loads_ordered(open(path, "rt").read())
    resolver = ref_resolver.RefResolver(dict_class=collections.OrderedDict)
    return io.SheetBuilder(resolver.resolve("file://" + path, sheet_json)).run()
